...
# Even with a dictionary, I still need to convert verb forms to dictionary form
# This should in fact be a separate module
        ...
        # a kanji with kun verb readings, one or more readings            
            ...
            if @rest {
                ...
                if $te-form {
                    ...
                    my ($kstr2,$kstr3,$ta-form) = ta-form-to-dict-form($kstr);
                    if $ta-form {
                        say "TA-FORM: $kstr => $kstr2 ($kstr3)" if $V;
                        if %dictionary{$kstr3}:exists {
                            return %dictionary{$kstr3}[0]
                        } else {
                            $*ERR.say( "Verb not in dictionary: " ~ $kstr ~ " ($kstr2,$kstr3)");
...
sub ta-form-to-dict-form (Str $kstr) {
# We need some more transformations for the te form
# if the final character is ta or da
# We can only do this for verbs with a single kanji because 
# the others are not in the lookup table
    my $kanji = $kstr.substr(0,1);
    if $kstr.chars == 3 and $kstr.substr(*-1) eq ('た'|’だ’) {
        if %dict-ending-for-ta-form{$kstr.substr(*-2,2)}:exists { # godan verb
            my @endings = %dict-ending-for-ta-form{$kstr.substr(*-2,2)};
            for @endings -> $ending {
                if %joyo_kun_verb_readings{$kanji}:exists {
                    my @kana = %joyo_kun_verb_readings{$kanji};
                        # For every reading 
    ...
    # When we have found the correct stem, we can append the ending. 
    # For our purpose it must be the stem though.
    } elsif $kstr.chars == 2 and $kstr.substr(*-1) eq 'た' { # ichidan butjust kanji+ru         
        if %joyo_kun_verb_readings{$kstr.substr(0,1)}:exists {
            my @kana = %joyo_kun_verb_readings{$kanji};
            # For every reading 
            for @kana -> $ks { 
...
} # END of ta-form-to-dict-form

# What about masu form?
# Ichidan:
# masu, mashita, mashite, mashou, masenka => ru
# Godan
# -tsu => -chi.masu
# -mu => -mi.masu
# -ku => -ki.masu
